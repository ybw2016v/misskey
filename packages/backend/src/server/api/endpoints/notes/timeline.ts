...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			...
			let exist = await this.redisTimelineService.isexist(ps.withFiles ? `homeTimelineWithFiles:${me.id}` : `homeTimeline:${me.id}`);
			...
			if (exist!=0){
				...
				redisTimelineService.keyexpire(ps.withFiles ? `homeTimelineWithFiles:${me.id}` : `homeTimeline:${me.id}`,60*60*24*7);
			...
			//#region Construct query
				...
				for (const note of timelineforredis) {
					this.redisTimelineService.pushall(ps.withFiles ? `homeTimelineWithFiles:${me.id}` : `homeTimeline:${me.id}`,note.id);
				}
